...
export const CreateUserSchema = z.object({
	...
	role: z.enum(['admin', 'staff', 'hoster', 'user']).default('user').optional(),
});
...
export const CreateOrganiserSchema = z.object({
	...
	role: z.enum(['admin', 'staff', 'hoster', 'user']).default('hoster'),
});

export const BecomeOrganiserSchema = z.object({
	// User fields
	email: z.email(),
	organisationName: z.string().min(1),
	phoneNumberVerified: z.boolean().default(false).optional(),
...
export const UpdateOrganiserSchema = z
	.object({
		address: AddressSchema.optional(),
		logo: z.url().optional(),
		bio: z.string().optional(),
...
/**
 * @description Zod schema for creating a payout account.
 */
export const CreateOrganizerPayoutAccountSchema = z.object({
	bankName: z.string().min(1, 'Bank name is required.'),
	bvn: z.string().min(1, 'Bank Verification Number is required.').optional(),
	bankAccountName: z.string().min(1, 'Account name is required.'),
	bankAccountNumber: z.string().min(1, 'Account number is required.'),
	bankRoutingNumber: z.string().min(1, 'Routing number is required.').optional(),
	schedulePayout: z.enum(['weekly', 'monthly']),
});

/**
 * @description Zod schema for updating a payout account.
 * All fields are optional to support partial updates.
 */
export const UpdateOrganizerPayoutAccountSchema = CreateOrganizerPayoutAccountSchema.partial();
...
export const PaginatedDateParamSchema = z
	.object({
		pagesize: z.preprocess((val) => {
			...
			if (!strVal || strVal.trim() === '') {
				throw new Error(
					'pagesize must be a valid number as a string. Received empty value.',
				);
			}
...
export const UpdateSocialAccountSchema = z
	.object({
		platform: z
			.enum(['facebook', 'twitterX', 'tiktok', 'youtube', 'instagram', 'linkedin'])
			.optional(),
...
export const CreateGoogleUserSchema = z.object({
	...
	name: z.string().min(1, 'Name is required.'),
	avatar: z.string().url('Image must be a valid URL.').nullable().optional(),
